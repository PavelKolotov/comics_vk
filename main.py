...
def main():
    ...
    vk_grup_id = env('GROUP_ID')
    vk_token = env('ACCESS_TOKEN')

    message = fetch_xkds_comics()

    server_link = get_wall_upload_server(vk_grup_id, vk_token)
    ...
    photo_link = save_wall_photo(vk_grup_id, vk_token, photo_param, server_param, hash_param)

    owner_id = photo_link['response'][0]['owner_id']
    media_id = photo_link['response'][0]['id']

    send_wall_post(vk_grup_id, vk_token, owner_id, media_id, message)
